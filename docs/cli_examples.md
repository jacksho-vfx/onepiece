...
- `chopper render scene.json --format png --s3-bucket <bucket> [--prefix <folder>] [--resume]` — render locally and stream the resulting frames or animation into S3, skipping any keys that already exist when resuming.
...
- `python -m apps.tester present [--host <host>] [--log-level <level>] [--open-browser/--no-browser] [--browser-path <alias>] [--browser-delay <seconds>] [--skip-create]` — run pipeline presentation hooks, seed demo datasets, and launch every dashboard plus the pipeline API surface. Use `--skip-create` to reuse existing fixtures without restaging them. 【F:src/apps/tester/app.py†L1-L220】
- `python -m apps.tester open [--host <host>] [--log-level <level>] [--open-browser/--no-browser]` — launch the demo dashboards and pipeline API without rerunning presentation hooks. Handy when iterating on UI changes or verifying copy updates. 【F:src/apps/tester/app.py†L221-L420】
