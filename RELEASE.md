...
This guide walks through every step to publish a new desktop release of OnePiece Studio. It assumes the repository is on GitHub and uses git tags like `v0.1.0`.

## 1) Update the version number
- Open the desktop app package file: `src/apps/ulti/package.json`.
- Bump the `"version"` field (for example: `0.1.0`, `0.2.0`, etc.). Stick to semantic-style versions: `MAJOR.MINOR.PATCH`.
...
git status
git add .
git commit -m "Bump desktop version to v0.1.0"
...
- `OnePiece Studio Desktop Setup 0.1.0.exe` (Windows)
- `OnePiece Studio Desktop-0.1.0.dmg` (macOS)
- `OnePiece Studio Desktop-0.1.0.AppImage` (Linux)

## 5) Create a GitHub release
1. Open the repository on GitHub.
2. Click **Releases** → **Draft a new release**.
3. Select the tag `v0.1.0` (create it if it is not listed yet).
4. Title the release (for example: `OnePiece Studio Desktop v0.1.0`).
