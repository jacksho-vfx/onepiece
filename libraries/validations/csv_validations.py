...
def validate_shots_csv(csv_path: Path) -> List[str]:
    ...
    with csv_path.open(newline="", encoding="utf-8") as f:
        ...
        for line_num, row in enumerate(reader, start=2):
            ...
            if not val:
                raise ValueError(f"Empty shot code in column '{col}' at line {line_num}.")
            if not pattern.match(val):
                raise ValueError(f"Invalid shot code format '{val}' at line {line_num}.")
            shot_codes.append(val)

    if not shot_codes:
        raise ValueError("No valid shot codes found in CSV.")

    log.info("validate_shots_csv_success",
             csv=str(csv_path), count=len(shot_codes))
    return shot_codes
