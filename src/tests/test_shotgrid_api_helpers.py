...
def test_get_or_create_episode_returns_existing(client: ShotGridClient) -> None:
    ...
    client.get_episode = MagicMock(return_value=episode)  # type: ignore[attr-defined]
    client.create_episode = MagicMock()  # type: ignore[attr-defined]

    result = client.get_or_create_episode(data)

    assert result == episode
    client.create_episode.assert_not_called()  # type: ignore[attr-defined]


def test_get_or_create_project_creates_when_missing(client: ShotGridClient) -> None:
    client.get_project = MagicMock(return_value=None)  # type: ignore[attr-defined]
    created = {"id": 5, "name": "New Project"}
    client.create_project = MagicMock(return_value=created)  # type: ignore[attr-defined]
...
def test_get_or_create_episode_skips_creation_without_identifiers(
    ...
    client.get_episode = MagicMock(side_effect=AssertionError("should not fetch"))  # type: ignore[attr-defined]
    client.create_episode = MagicMock(side_effect=AssertionError("should not create"))  # type: ignore[attr-defined]
...
def test_get_or_create_scene_creates_when_missing(client: ShotGridClient) -> None:
    ...
    client.get_scene = MagicMock(return_value=None)  # type: ignore[attr-defined]
    client.create_scene = MagicMock(return_value=created)  # type: ignore[attr-defined]
...
def test_get_or_create_shot_skips_creation_without_identifiers(
    ...
    client.get_shot = MagicMock(side_effect=AssertionError("should not fetch"))  # type: ignore[attr-defined]
    client.create_shot = MagicMock(side_effect=AssertionError("should not create"))  # type: ignore[attr-defined]
