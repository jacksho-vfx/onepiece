...
class _FakeShotGridClient:
    ...
    def get_project(self, name: str):  # pragma: no cover - behaviour tested via setup
        return self._project

    def get_or_create_episode(self, payload):
        self.episode_payload = payload
        return {"id": 101}

    def get_or_create_scene(self, payload):
        self.scene_payload = payload
        return {"id": 202}

    def get_or_create_shot(self, payload):
...
def test_setup_single_shot_missing_project_id() -> None:
    ...
    with pytest.raises(RuntimeError, match="ShotGrid project 'my-show' is missing an id"):
        setup_single_shot("my-show", "E01_S01_SH010", client=client)
...
def test_setup_single_shot_uses_project_id() -> None:
    ...
    assert client.episode_payload.project_id == 777
    assert client.scene_payload.project_id == 777
    assert client.shot_payload.project_id == 777
