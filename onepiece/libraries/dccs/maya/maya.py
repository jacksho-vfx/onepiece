...
def open_scene(path: UPath):
    ...
    pm.openFile(str(path), force=True)
...
def import_asset(path: UPath):
    ...
    pm.importFile(str(path), type="mayaAscii" if path.endswith(".ma") else "mayaBinary")
    log.info("maya_asset_imported", path=str(path))


def export_scene(path: UPath):
    """
