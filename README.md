...
current dashboard build number directly from the CLI. Automated cost
recommendations ship alongside the dashboard: `perona cost insights` analyses
recent telemetry and surfaces optimisation advice, while
[`GET /api/cost/insights`](docs/perona_dashboard.md#cost-insights-api) exposes
...
- **Delivery manifest optimisation** – Delivery payloads prefer upstream manifest data when provided, regenerate manifests only once per delivery, and gracefully handle packages that arrive without entry lists so operators still get a full audit trail. Parallel package staging now batches copy/link work using worker pools that honour the `profiles.<name>.dcc.copy_workers` profile key or the `ONEPIECE_DCC_COPY_WORKERS` environment variable when you need to tune concurrency.
